...
<div class="container p-5">
    ...
    <div class="content">
        <h1 class="max-w-2xl mb-4 font-semibold tracking-tight leading-none text-3xl md:text-5xl xl:text-4xl dark:text-white" style="color: #2C6775;">
            {{$servicedetail->service_name }}
        </h1>
        <span class="bg-cyan-500 shadow-lg shadow-cyan-500/50 p-2 text-xs rounded-full"><i class="fa fa-briefcase pr-2 "></i> {{$servicedetail->cat->category }}</span>
        <p style="color: #2C6775;" class="mt-3 tracking-tight my-2 lead">from <span class=" font-semibold text-sm/11">៛ {{$servicedetail->service_price }}</span> </p>
        <h2 class="mr-auto tracking-tight my-2 lead">
            <span style="color: #2C6775;" class="font-semibold">Service</span> by
            <a style="color:#1B9C85" href="/company_detail" class="underline font-bold"> {{$servicedetail->comp->company_name }}</a>
